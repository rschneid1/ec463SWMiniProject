...
firebase.auth().onAuthStateChanged( user => {
  if (user) {
    ...
    provider  = user.providerId;
	  ...
	  //console.log(userID);
	  usersRef.doc(userID).get().then((docSnapshot) => {
		 if(!docSnapshot.exists) {
			usersRef.doc(userID).set(data);
			usersRef.doc(userID).collection('Tests').doc('test0').set(test0);
		 }
	  })
...
class Dashboard extends React.Component {
  ...
  render() {
	const usersRef = firebase.firestore().collection('users');  
	var testRef = usersRef.doc(email).collection('Tests');
	//var testName = usersRef.doc(email).get().then(function(doc) {
						//testName = 'test';
						//var testNumber = doc.get('testNumber');
						//testName = test.concat(testNumber);
						//return testName;
					//})
	//var recentTest = testRef.doc(testName).get().then(function(doc) {
							//recentTest = doc.get('Date');
							//return recentTest;
						//})
    return (
      <SafeAreaView style={styles.container}>
		...
		<Separator />
		<View>
			<Text>Most Recent Covid Symptoms Test</Text>
		</View>
